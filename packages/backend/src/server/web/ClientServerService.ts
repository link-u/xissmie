...
export class ClientServerService {
	...
	public createServer(fastify: FastifyInstance, options: FastifyPluginOptions, done: (err?: Error) => void) {
		...
		fastify.get<{ Params: { emoji: string; } }>('/xissmie/store/emojis/:emoji', async (request, reply) => {
			...
			reply.redirect(`https://${this.config.xfolioApiHost}/portfolio/${emoji.storeAuthorId}/shop/${emoji.storeProductId ?? emoji.id}`);
		});
		...
		fastify.get<{ Params: { decoration: string; } }>('/xissmie/store/avatar-decorations/:decoration', async (request, reply) => {
			...
			reply.redirect(`https://${this.config.xfolioApiHost}/portfolio/${decoration.storeAuthorId}/shop/${decoration.storeProductId ?? decoration.id}`);
		});
